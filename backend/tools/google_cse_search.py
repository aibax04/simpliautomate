        ...
        with DDGS() as ddgs:
            ...
            for r in ddg_results:
                results.append({
                    ...
                    "snippet": r.get("body", ""),
                    "link": r.get("href", ""),
                    "displayLink": r.get("href", "").split("//")[-1].split("/")[0]
                })
...
def search_google_cse(query: str, max_results: int = 10) -> List[Dict[str, str]]:
    ...
    try:
        ...
        # If we hit rate limit (429) or other errors, fallback to DDG
        if response.status_code == 429:
            logger.warning(f"Google CSE rate limit reached (429). Switching to DuckDuckGo.")
            return search_ddg(query, max_results)
