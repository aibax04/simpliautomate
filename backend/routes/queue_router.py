...
@router.post("/update-post-caption")
async def update_post_caption(payload: Dict[str, Any], user: User = Depends(get_current_user)):
    """
    Updates the caption, hashtags, and caption_data for a post after regeneration.
    """
    post_id = payload.get("post_id")
    caption = payload.get("caption")
    hashtags = payload.get("hashtags")
    caption_data = payload.get("caption_data")
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            if db_post:
                ...
                # If we had a hashtags column, we'd update it here.
                # Currently caption includes everything in most models.
                ...
                try:
                    ...
                    if db_job:
                        new_result = dict(db_job.result_json)
                        new_result["text"] = caption
                        if caption_data:
                            new_result["caption_data"] = caption_data
                        db_job.result_json = new_result
                        print(f"[DEBUG] Synced caption update to GenerationQueue {db_job.id}")
                except Exception as sync_e:
                    print(f"[WARNING] Syncing to queue history failed: {sync_e}")

                await session.commit()
                print(f"[SUCCESS] Updated post {db_post.id} with new caption.")
                return {"status": "success", "message": "Post caption updated successfully"}
            else:
                raise HTTPException(status_code=404, detail="Post not found to update")
        except Exception as e:
            await session.rollback()
            print(f"[ERROR] update-post-caption failed: {e}")
            raise HTTPException(status_code=500, detail=str(e))
